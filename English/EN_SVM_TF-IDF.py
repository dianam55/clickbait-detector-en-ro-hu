...
df_Clickbait_and_NonClickbait['Headline'] = df_Clickbait_and_NonClickbait['Headline'].apply(lambda x: re.sub(r"[^a-zA-Z\s.,!?;:]", '', x)) #keeps letters, spaces, punctuation
...
df_Clickbait_and_NonClickbait['tokens'] = df_Clickbait_and_NonClickbait['tokens'].apply(lambda x: [word for word in x if word not in stop_words]) #removes stop words
...
#TF-IDF 
tfidf_vectorizer = TfidfVectorizer(max_features=5000, ngram_range=(1, 2))  #both unigrams and bigrams
...
grid_search = GridSearchCV(svm_model, param_grid, cv=5, scoring='accuracy', n_jobs=-1) ##n_jobs=-1 use all available CPU cores
